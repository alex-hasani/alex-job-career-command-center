...
function publicFastApplyPending(pending) {
  return pending ? { queueId:pending.queueId, status:pending.status, queuedAt:pending.queuedAt, sendAt:pending.sendAt, to:pending.input.to, error:pending.error || '', delivery:publicFastApplyDelivery(pending.delivery) } : null;
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/fast-apply/pending' && req.method === 'GET') {
      const job=await applicationJob(url.searchParams.get('id'));
      res.writeHead(200, {'content-type':'application/json','cache-control':'no-store'});
      return res.end(JSON.stringify({ok:true,delivery:publicFastApplyDelivery(readFastApplyDelivery(job.id)),pending:publicFastApplyPending(pendingFastApplySends.get(job.id))}));
    }
    if (url.pathname === '/api/fast-apply/undo' && req.method === 'POST') {
      const input=await jsonBody(req), job=await applicationJob(input.id), pending=pendingFastApplySends.get(job.id);
      if (!pending || pending.status !== 'queued') throw new Error('There is no queued Fast Apply email to cancel. Gmail emails already sent cannot be recalled.');
      clearTimeout(pending.timer); pending.status='cancelled'; pendingFastApplySends.delete(job.id);
      logActivity({ action:'fast_apply.undo', result:'cancelled', jobId:job.id, detail:'Queued email cancelled before Gmail delivery' });
      res.writeHead(200, {'content-type':'application/json','cache-control':'no-store'});
...
async function restartWhenApplicationCodeChanges() {
  ...
  // A Fast Apply queue is deliberately short-lived, but Gmail delivery must
  // never be lost because the development watchdog reloads the app mid-window.
  if (hasActiveFastApplySend()) {
    console.log('Alex Job code changed; deferring restart until the active Fast Apply delivery finishes.');
    return;
  }
