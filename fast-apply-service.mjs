...
function candidates(job,posting='') {
  const values=[];
  // The retrieved job description is the source of truth. A saved recruiter contact may be stale or belong to a different listing.
  for (const [text,source] of [[posting,'verified job description'],[job?.recruiterContact,'saved recruiter contact']]) {
...
function postingRecipients(posting='') { return candidates(null,posting).map(item=>item.email); }
